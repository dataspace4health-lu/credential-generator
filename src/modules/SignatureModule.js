...
export class SignatureModule {
  ...
  async getOrGenerateKeyPair(algorithm = "EdDSA") {
  ...
  async signCredentialWithExistingProofs(
    ...
    newProof.id = matchingProofs?.length > 0 ? "urn:uuid:" + uuidv4() : undefined;
    // newProof.previousProof = lastProof.id;
    newProof.previousProof = matchingProofs?.length > 1 ? 
      matchingProofs?.map(p => p.id) : matchingProofs?.[0]?.id;
